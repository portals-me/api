...
export default {
  ...
  methods: {
    ...
    async signInWithTwitter () {
      const twitterAuthURL = await axios.post(`${this.$config.API}/auth/twitter`);
    ...
    async signInWithTwitterAfter (token) {
      try {
        const credential = (await axios.get(`${this.$config.API}/auth/twitter?oauth_token=${token.oauth_token}&oauth_verifier=${token.oauth_verifier}`)).data.credential;
        const result = (await sdk.signIn({
          twitter: credential,
        })).data;
    ...
    async signUpWithTwitterAfter (token) {
      const result = (await axios.get(`${this.$config.API}/auth/twitter?oauth_token=${token.oauth_token}&oauth_verifier=${token.oauth_verifier}`)).data;
